...
public class TemporaryAssignmentServiceTest {
    ...
    @Test
    public void testSameStaffForReplacementThrowsException() {
        StaffResponseDto regular = createStaff("STF-TMP-003", "Same Worker");

        TemporaryAssignmentCreateRequestDto req = new TemporaryAssignmentCreateRequestDto();
        req.setRegularStaffId(regular.getId());
        req.setReplacementStaffId(regular.getId());
        req.setStartDate(LocalDate.now());
        req.setEndDate(LocalDate.now().plusDays(2));

        assertThrows(InvalidOperationException.class, () -> {
            tempService.createAssignment(req);
        });
    }
}
